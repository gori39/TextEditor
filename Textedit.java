    ...
    public void actionPerformed(ActionEvent e){
        ...
        if(s.equals("New")){
            ...
            if(r==JFileChooser.APPROVE_OPTION){
                ...
                //we dont want bufferred to be fulled and if it hold it previous it cant hold new so we need to flush the old one,cleared the stream
                bw.flush();
                bw.close(); //deleting the bufferwriter,so that fresh bufferwriter can be created
                }
                catch(Exception et){
                JOptionPane.showMessageDialog(f, et.getMessage()); //if any error is there it will be displayed 
            }
